...
# --- ZIP / Borough lookup (generated by data/build_zip_borough.py) ---
_ZIP_BOROUGH_PATH = Path(__file__).resolve().parent.parent / "data" / "nyc_zip_borough.json"
with open(_ZIP_BOROUGH_PATH) as _f:
    _ZIP_DATA = json.load(_f)
VALID_ZIPS: set[str] = set(_ZIP_DATA["valid_zips"])
ZIP_TO_BOROUGH: dict[str, str] = _ZIP_DATA["zip_borough"]
...
def get_borough_from_zip(z: str) -> str:
    return ZIP_TO_BOROUGH.get(str(z), "Unknown")
...
with tab_pred:
    ...
    with col2:
        ...
        with st.container(border=True):
            ...
            if predict_button:
                if not zip_code:
                    st.error("Please enter a 5-digit NYC ZIP Code.")
                elif zip_code not in VALID_ZIPS:
                    st.error(f"{zip_code} is not a valid NYC ZIP code covered by this model. Please enter one of the 177 supported NYC ZIP codes.")
                else:
            ...
            if apply_filters:
                ...
                if zip_filter:
                    if zip_filter not in VALID_ZIPS:
                        st.error(f"{zip_filter} is not a valid NYC ZIP code covered by this model.")
                        valid = False
                    elif borough_filter != "All":
                        zip_borough = get_borough_from_zip(zip_filter)
                        if zip_borough != borough_filter:
